...
ci_daily_sequence <- read.csv("outputs/ci_daily_seq.csv",header = FALSE) 
ci_daily_sequence <- t(ci_daily_sequence)
ci_daily_sequence <- data.frame(ci_daily_sequence)%>%
  mutate(coln = seq(1,2173, 1))
colnames(ci_daily_sequence) <- c("n","dd","d","all","ratio","coln")
a <- read.csv("outputs/ci_daily_seq_dates.csv", header = FALSE) %>%
  mutate(coln = seq(1, 2173, 1))
ci_daily_sequence <- ci_daily_sequence %>%
  left_join(a) %>%
  mutate(date = V1) %>%
  select(-V1, -coln)
ci_daily_sequence$date <- as.Date(ci_daily_sequence$date, format = "%d-%b-%Y")

## fill in missing dates w/ Nans
ci_daily_sequence <- pad(ci_daily_sequence)
ci_daily_sequence$n[is.nan(ci_daily_sequence$n)] <- NA
ci_daily_sequence$d[is.nan(ci_daily_sequence$d)] <- NA
ci_daily_sequence$dd[is.nan(ci_daily_sequence$dd)] <- NA
ci_daily_sequence$all[is.nan(ci_daily_sequence$all)] <- NA
ci_daily_sequence$ratio[is.nan(ci_daily_sequence$ratio)] <- NA

## running mean
ci_daily_sequence$ratio_rm <- rollapply(ci_daily_sequence$ratio,windowsize,mean,fill=NA,na.rm = TRUE)
ci_daily_sequence$ratio_rm[1:209] <- NA # correct for running mean creating "artificial" ratio values before the first date of recording (July 29, 2015)
ci_daily_sequence$all_rm <- rollapply(ci_daily_sequence$all,windowsize,mean,fill=NA,na.rm = TRUE)
ci_daily_sequence$all_rm[1:209] <- NA # correct for running mean creating "artificial" values before the first date of recording (July 29, 2015)
ci_daily_sequence$n_rm <- rollapply(ci_daily_sequence$n,windowsize,mean,fill=NA,na.rm = TRUE)
ci_daily_sequence$n_rm[1:209] <- NA # correct for running mean creating "artificial" values before the first date of recording (July 29, 2015)
ci_daily_sequence$d_rm <- rollapply(ci_daily_sequence$d,windowsize,mean,fill=NA,na.rm = TRUE)
ci_daily_sequence$d_rm[1:209] <- NA # correct for running mean creating "artificial" values before the first date of recording (July 29, 2015)
...
for (d in (1):(2223-windowsize_ttest)) {
  if (d < 209+windowsize_ttest) {
    ...
    s1 <- ci_daily_sequence$ratio_rm[i1:i2]
    s2 <- ci_daily_sequence$ratio_rm[j1:j2]
...
ci_daily_sequence$pv1 <- dci_sig1$pv
ci_daily_sequence$pv2 <- dci_sig2$pv

## only consider CInight:CIday for days with blue whale song presence (CI >= 1.01)
ci_daily_sequence$pv1[ci_daily_sequence$all_rm < 1.01] <- NA
ci_daily_sequence$pv2[ci_daily_sequence$all_rm < 1.01] <- NA
ci_daily_sequence$ratio_rm[ci_daily_sequence$all_rm < 1.01] <- NA
...
increase<- ci_daily_sequence %>%
  filter(pv2<0.05)
decrease<- ci_daily_sequence %>%
  filter(pv1<0.05)
song <- ci_daily_sequence %>%
  filter(all_rm>1.01)
...
## generate Figure 2
tiff("outputs/alt.tiff",units="in", width=10,height=4,res=300)
# panel A
pa <- ci_daily_sequence %>%
